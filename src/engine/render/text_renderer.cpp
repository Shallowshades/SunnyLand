...
{
	if (!mSdlRenderer || !mResourceManager) {
	...
	mTextEngine = TTF_CreateRendererTextEngine(mSdlRenderer);
	if (!mTextEngine) {
...
void TextRenderer::drawUIText(const std::string& text, const std::string& font_id, int font_size, const glm::vec2& position, const engine::utils::FColor& color) {
	...
	TTF_Text* temp_text_object = TTF_CreateText(mTextEngine, font, text.c_str(), 0);
	if (!temp_text_object) {
